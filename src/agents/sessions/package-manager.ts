...
export class DefaultPackageManager implements PackageManager {
  ...
  private async updateNpmBatch(
    ...
    const message =
      sources.length === 1
        ? `Updating ${sources[0].source}...`
        : `Updating ${scope} npm packages...`;
  ...
  private findSuggestedConfiguredSource(
    ...
    for (const pkg of configuredPackages) {
      ...
      if (parsed.type === "git") {
        ...
        if (
          trimmedSource === shorthand ||
          (shorthandWithRef && trimmedSource === shorthandWithRef)
        ) {
          suggestions.add(sourceStr);
        }
      }
  ...
  private async getRemoteGitHead(installedPath: string): Promise<string> {
    ...
    if (upstreamRef) {
      const remoteHead = await this.runGitRemoteCommand(installedPath, [
        "ls-remote",
        "origin",
        upstreamRef,
      ]);
  ...
  private async getLocalGitUpdateTarget(
    ...
    try {
      ...
      const originHeadRef = await this.runCommandCapture(
        "git",
  ...
  private getPackageManagerName(): string {
    ...
    const packageManagerCommand =
      separatorIndex >= 0 ? commandParts[separatorIndex + 1] : npmCommand.command;
  ...
  private ensureNpmProject(installRoot: string): void {
    ...
    if (!existsSync(packageJsonPath)) {
      const pkgJson = { name: "openclaw-resources", private: true };
  ...
  private getLegacyGlobalNpmInstallPath(source: NpmSource): string | undefined {
    try {
      return (
        this.getPnpmGlobalPackagePath(source.name) ?? join(this.getGlobalNpmRoot(), source.name)
      );
    } catch {
      return undefined;
    }
  ...
  private runCommandCapture(
    ...
    return new Promise((resolvePromise, reject) => {
      ...
      child.once("close", (code, signal) => {
        ...
        reject(
          new Error(`${command} ${args.join(" ")} failed with ${exitStatus}: ${stderr || stdout}`),
        );
      });
    });
  }
